...
|                                                                                  | 简单承诺 (摘要/哈希) | 时间戳 | 一次性密封 |
| -------------------------------------------------------------------------------- | ------------------ | ------ | --------- |
| 承诺的公布不会透露消息                                                          | 是                 | 是      | 是         |
| 承诺日期 / 消息在特定日期之前存在的证明                                         | 不可能             | 可能    | 可能       |
| 证明不存在其他替代承诺                                                          | 不可能             | 不可能  | 可能       |
...
| 方法                      | 链上踪迹与大小 | 客户端大小 | 钱包集成 | 硬件兼容性 | Lightning 兼容性       | Taproot 兼容性          |
| ----------------------- | ------- | ----- | ---- | ----- | ------------------- | -------------------- |
| Keytweak（确定性 P2C）       | 🟢      | 🟡    | 🔴   | 🟠    | 🔴 BOLT, 🔴 Bifrost | 🟠 Taproot, 🟢 MuSig |
| Sigtweak（确定性 S2C）       | 🟢      | 🟢    | 🟠   | 🔴    | 🔴 BOLT, 🔴 Bifrost | 🟠 Taproot, 🔴 MuSig |
| Opret（OP_RETURN）        | 🔴      | 🟢    | 🟢   | 🟠    | 🔴 BOLT, 🟠 Bifrost | -                    |
| Tapret 算法：左上节点          | 🟠      | 🔴    | 🟠   | 🟢    | 🔴 BOLT, 🟢 Bifrost | 🟢 Taproot, 🟢 MuSig |
| Tapret 算法 # 4：任意节点 + 证明 | 🟢      | 🟠    | 🟠   | 🟢    | 🔴 BOLT, 🟢 Bifrost | 🟢 Taproot, 🟢 MuSig |

| 确定性承诺方案                    | 标准             | 链上成本                                         | 客户端证明大小                                                          |
| -------------------------- | -------------- | -------------------------------------------- | ---------------------------------------------------------------- |
| Keytweak（确定性 P2C）          | LNPBP-1, 2     | 0 字节                                         | 33 字节（未调整的密钥）                                                    |
| Sigtweak（确定性 S2C）          | WIP (LNPBP-39) | 0 字节                                         | 0 字节                                                             |
| Opret（OP_RETURN）           | -              | 36 (v)字节（额外的 TxOut）                          | 0 字节                                                             |
| Tapret 算法：左上角节点            | LNPBP-6        | 32 字节的见证（8 v字节），适用于任何 n-of-m 多重签名，并通过脚本路径支出  | 0 字节的 scriptless scripts taproot ~270 字节（单个脚本情况），~128 字节（多个脚本情况） |
| Tapret 算法 # 4：任意节点 + 唯一性证明 | LNPBP-6        | 32 字节的见证（8 v字节），适用于单个脚本情况，在大多数其他情况下见证中为 0 字节 | 0 字节的 scriptless scripts taproot，65 字节直到 Taptree 包含十二个脚本         |

| 层级                            | 链上成本 (bytes/vbytes) | 链上成本 (bytes/vbytes) | 链上成本 (bytes/vbytes) | 链上成本 (bytes/vbytes) | 链上成本 (bytes/vbytes) | 客户端成本 (bytes) | 客户端成本 (bytes) | 客户端成本 (bytes) | 客户端成本 (bytes) | 客户端成本 (bytes) |
| ------------------------------ | -------------------- | -------------------- | -------------------- | -------------------- | -------------------- | ---------------- | ---------------- | ---------------- | ---------------- | ---------------- |
| **类型**                       | **Tapret**           | **Tapret #4**        | **Keytweak**         | **Sigtweak**         | **Opret**            | **Tapret**       | **Tapret #4**    | **Keytweak**     | **Sigtweak**     | **Opret**        |
| 单签名                         | 0                    | 0                    | 0                    | 0                    | 32                   | 0                | 0                | 32               | 0?               | 0                |
| MuSig (n-of-n)                 | 0                    | 0                    | 0                    | 0                    | 32                   | 0                | 0                | 32               | ? > 0            | 0                |
| 多重签名 2-of-3                | 32/8                 | 32/8 或 0            | 0                    | n/a                  | 32                   | ~270             | 65               | 32               | n/a              | 0                |
| 多重签名 3-of-5                | 32/8                 | 32/8 或 0            | 0                    | n/a                  | 32                   | ~340             | 65               | 32               | n/a              | 0                |
| 带超时的多重签名 2-of-3        | 32/8                 | 0                    | 0                    | n/a                  | 32                   | 64               | 65               | 32               | n/a              | 0                |


| 层级                            | 链上成本 (vbytes) | 链上成本 (vbytes) | 链上成本 (vbytes) | 客户端成本 (bytes) | 客户端成本 (bytes) |
| ------------------------------ | ---------------------- | ---------------------- | ---------------------- | ------------------------ | ------------------------ |
| **类型**                       | **基础**               | **Tapret #2**          | **Tapret #4**          | **Tapret #2**            | **Tapret #4**            |
| MuSig (n-of-n)                 | 16.5                   | 0                      | 0                      | 0                        | 0                        |
| FROST (n-of-m)                 | ?                      | 0                      | 0                      | 0                        | 0                        |
| Multi_a (n-of-m)               | 1+16n+8m               | 8                      | 8                      | 33 * m                   | 65                       |
| MuSig / Multi_a 分支 (n-of-m)   | 1+16n+8n+8xlog(n)      | 8                      | 0                      | 64                       | 65                       |
| 具有超时 (n-of-m)              | 1+16n+8n+8xlog(n)      | 8                      | 0                      | 64                       | 65                       |

| 方法                        | 隐私与可扩展性 | 互操作性 | 兼容性 | 可移植性 | 复杂性 |
| ------------------------- | ------- | ---- | --- | ---- | --- |
| Keytweak（确定性 P2C）         | 🟢      | 🔴   | 🔴  | 🟡   | 🟡  |
| Sigtweak（确定性 S2C）         | 🟢      | 🔴   | 🔴  | 🟢   | 🔴  |
| Opret（OP_RETURN）          | 🔴      | 🟠   | 🔴  | 🟢   | 🟢  |
| Algo Tapret：左上节点          | 🟠      | 🟢   | 🟢  | 🔴   | 🟠  |
| Algo Tapret # 4：任意节点 + 证明 | 🟢      | 🟢   | 🟢  | 🟠   | 🔴  |
...
|                                 | 元数据                                   | 全局状态                                  | 拥有状态                                                                                                 |
| ------------------------------- | -------------------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| **范围**                        | 仅适用于单个合约操作                     | 全局定义适用于整个合约                   | 适用于每个密封 (*Assignment*)                                                                      |
| **谁可以更新？**                 | 不可更新（临时数据）                     | 由参与者（发行人等）执行的操作             | 取决于合法持有密封的人（可以在下一笔交易中花费的人）                                              |
| **时间范围**                     | 仅适用于当前操作                         | 操作结束时确定状态                        | 状态在操作之前定义（由前一个操作的 *Seal Definition* 确定）                                        |
...
| **元素**          | **声明式**      | **可替代**                            | **结构化**                      | **附件**                        |
| ----------------- | -------------- | ------------------------------------ | ----------------------------- | ----------------------------- |
| **数据**         | 无              | 64位有符号或无符号整数                | 任意严格数据类型                | 任意文件                        |
| **信息类型**     | 无              | 有符号或无符号                        | 严格类型                        | MIME 类型                      |
| **隐私**         | 无需            | Pedersen commitment                  | 盲化哈希                        | 哈希文件标识符                  |
| **大小限制**     | N/A            | 256 字节                              | 最大 64 KB                      | 最大 ~500 GB                    |
...
| RGB 合约组件                 | 含义                              | OOP 等效                             | Ethereum 等效                      |
| ---------------------------- | -------------------------------- | ----------------------------------- | --------------------------------- |
| **Genesis**                  | 合约的初始状态                   | 类构造函数                          | 合约构造函数                       |
| **Schema**                   | 合约的业务逻辑                   | 类                                  | 合约                             |
| **Interface**                | 合约的语义                       | 接口 (Java) / Trait (Rust) / 协议 (Swift) | ERC 标准                         |
| **Interface Implementation** | 语义和逻辑的映射                 | Impl (Rust) / Implements (Java)    | 应用程序二进制接口 (ABI)           |
